    ...
    if (cpt < maxCpt) {
      ...
      Object.keys(matches[matchInd].matchData.metadata).forEach(function (arrayItem) {
        ...
        Object.keys(matches[matchInd].matchData.metadata[arrayItem]['text'].position).forEach(function (position) {
          posArray.push([matches[matchInd].matchData.metadata[arrayItem]['text'].position[position][0] + offset, matches[matchInd].matchData.metadata[arrayItem]['text'].position[position][1], arrayItem]);
          cpt = cpt + 1;
        });
      });
      ...
      var newPosArrayNoDup = uniqByKeepFirst(newPosArray, function (it) {
        return it[0];
      });
      ...
      Object.keys(newPosArrayNoDup).forEach(function (arrayItem) {
        ...
        resultContent = resultContent + "<div class=\"web_match\" onclick=\"showProductInfo('" + id + "')\">" + textStartFullStop + MatchFormattedBefore + "<span class='web_search-marker'>" + MatchFormatted + "</span>" + MatchFormattedAfter + textEndFullStop + "</div>"; //Build match in content
...
var getTextNodesIn = function getTextNodesIn(el) {
